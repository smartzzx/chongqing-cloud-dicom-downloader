...
def suggest_save_dir(patient: str, desc: str, datetime: str, base: str = "download"):
	"""
	统一的函数用来确定影像的保存位置，名字一律为：[时间]-[患者]-[检查]
	患者姓名可能有星号代替，所以也需要 `pathify` 一下。

	:param patient: 患者名字
	:param desc: 检查项目
	:param datetime: 检查时间，尽量包含从年份到秒
	:param base: 保存的根目录，默认 download
	"""
	patient, desc = pathify(patient), pathify(desc)
	datetime = TIME_SEPS.sub("", datetime)
	return Path(base) / f"{datetime}-{patient}-{desc}"
